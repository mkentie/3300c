/*
  ...
  $Id: niash.c,v 1.14 2006/02/04 10:42:09 ullsig Exp $
*/
...
#define ASSERT(cond) (!(cond) ? NIASHDBG(DBG_ASSERT, "!!! ASSERT(%S) FAILED!!!\n",STRINGIFY(cond));)
...
_WaitForLamp (TScanner * s, unsigned char *pabCalibTable)
{
  ...
  NIASHDBG (DBG_MSG, "_WaitForLamp: first calibration\n");
  ...
  while (s->WarmUpTime)
    {
      ...
      if (s->WarmUpTime && fHasCal && iCal)
        {
          ...
          if (_TimeElapsed
              (&s->WarmUpStarted, &now[iCurrent], WARMUP_MAXTIME))
            {
              /* stop idling */
              s->WarmUpTime = 0;
              NIASHDBG (DBG_MSG, "_WaitForLamp: WARMUP_MAXTIME=%ds elapsed!\n",
                   WARMUP_MAXTIME);
            }
        }
      ...
      if (s->WarmUpTime)
        {
          /* if the (too low) calibration values have just been acquired
             we start waiting */
          if (fHasCal)
            NIASHDBG (DBG_MSG, "_WaitForLamp: entering delay loop\r");
          else
            NIASHDBG (DBG_MSG, "_WaitForLamp: delay loop %d        \r", ++iDelay);
          sleep (1);
          fHasCal = SANE_FALSE;
          gettimeofday (&now[!iCurrent], 0);
        }
      ...
      if (s->WarmUpTime         /* did we have to wait at all */
                           ...
                           WARMUP_TESTINTERVAL))
        {
          ...
          NIASHDBG (DBG_MSG, "_WaitForLamp: recalibration #%d, deviation = %d%%\n",
               iCal, dev);
  ...
  for (i = optCount; i < optLast; i++)
    {
      ...
      switch (i)
        ...
        default:
          NIASHDBG (DBG_ERR, "Uninitialised option %d\n", i);
          break;
        }
    }
}
...
_ReportDevice (TScannerModel * pModel, const char *pszDeviceName)
{
  ...
  NIASHDBG (DBG_MSG, "niash: _ReportDevice '%s'\n", pszDeviceName);

  pNew = malloc (sizeof (TDevListEntry));
  if (!pNew)
    {
      NIASHDBG (DBG_ERR, "no mem\n");
      return -1;
    }
...
sane_get_devices (const SANE_Device *** device_list, SANE_Bool local_only)
{
  ...
  NIASHDBG (DBG_MSG, "sane_get_devices\n");
...
sane_open (SANE_String_Const name, SANE_Handle * h)
{
  ...
  if (!s)
    {
      NIASHDBG (DBG_MSG, "malloc failed\n");
      return SANE_STATUS_NO_MEM;
    }
  ...
  if (NiashOpen (&s->HWParams, name) < 0)
    {
      /* is this OK ? */
      NIASHDBG (DBG_ERR, "NiashOpen failed\n");
      free ((void *) s);
...
sane_get_option_descriptor (SANE_Handle h, SANE_Int n)
{
  ...
  NIASHDBG (DBG_MSG, "sane_get_option_descriptor %d\n", n);
                     ...
                     void *pVal, SANE_Int * pInfo)
{
  ...
  NIASHDBG (DBG_MSG, "sane_control_option: option %d, action %d\n", n, Action);
  ...
  switch (Action)
    ...
    case SANE_ACTION_GET_VALUE:
      switch (n)
        ...
        case optGammaTable:
          NIASHDBG (DBG_MSG, "Reading gamma table\n");
          memcpy (pVal, s->aValues[n].wa, s->aOptions[n].size);
          break;

        case optMode:
          NIASHDBG (DBG_MSG, "Reading scan mode %s\n",
        ...
        default:
          NIASHDBG (DBG_MSG, "SANE_ACTION_GET_VALUE: Invalid option (%d)\n", n);
        }
      break;
      ...
      switch (n)
        ...
        case optBRY:
          ...
          if (status != SANE_STATUS_GOOD)
            {
              NIASHDBG (DBG_ERR, "Failed to constrain option %d (%s)\n", n,
                   s->aOptions[n].title);
              return status;
            }
        ...
        case optGammaTable:
          NIASHDBG (DBG_MSG, "Writing gamma table\n");
          pi = (SANE_Int *) pVal;
        ...
        case optMode:
          {
            ...
            NIASHDBG (DBG_MSG, "setting scan mode: %s\n", (char const *) pVal);
          }
          break;
...
/*       SimpleCalib(&s->HWParams); */
        ...
        default:
          NIASHDBG (DBG_ERR, "SANE_ACTION_SET_VALUE: Invalid option (%d)\n", n);
        }
      if (pInfo != NULL)
        {
          *pInfo |= info;
    ...
    default:
      NIASHDBG (DBG_ERR, "Invalid action (%d)\n", Action);
      return SANE_STATUS_INVAL;
    }
...
sane_get_parameters (SANE_Handle h, SANE_Parameters * p)
{
  ...
  NIASHDBG (DBG_MSG, "sane_get_parameters\n");
  ...
  if (s->aValues[optTLX].w >= s->aValues[optBRX].w)
    {
      NIASHDBG (DBG_ERR, "TLX should be smaller than BRX\n");
      return SANE_STATUS_INVAL; /* proper error code? */
    }
  if (s->aValues[optTLY].w >= s->aValues[optBRY].w)
    {
      NIASHDBG (DBG_ERR, "TLY should be smaller than BRY\n");
      return SANE_STATUS_INVAL; /* proper error code? */
    }
...
sane_start (SANE_Handle h)
{
  ...
  NIASHDBG (DBG_MSG, "sane_start\n");

  s = (TScanner *) h;

  if (sane_get_parameters (h, &par) != SANE_STATUS_GOOD)
    {
      NIASHDBG (DBG_MSG, "Invalid scan parameters\n");
      return SANE_STATUS_INVAL;
    }
...
sane_read (SANE_Handle h, SANE_Byte * buf, SANE_Int maxlen, SANE_Int * len)
{
  ...
  NIASHDBG (DBG_MSG, "sane_read: buf=%p, maxlen=%d, ", buf, maxlen);
  ...
  if (!s->fScanning)
    {
      if (s->fCancelled)
        {
          NIASHDBG (DBG_MSG, "\n");
          NIASHDBG (DBG_MSG, "sane_read: sane_read cancelled\n");
          s->fCancelled = SANE_FALSE;
  ...
  if ((s->iLinesLeft == 0) && (s->iBytesLeft == 0))
    {
      ...
      *len = 0;
      NIASHDBG (DBG_MSG, "\n");
      NIASHDBG (DBG_MSG, "sane_read: end of scan\n");
      s->fCancelled = SANE_FALSE;
      s->fScanning = SANE_FALSE;
  ...
  if (s->iBytesLeft == 0)
    {
      ...
      /* stop scanning further, when the read action fails
      ...
      else
        {
          ...
          *len = 0;
          NIASHDBG (DBG_MSG, "\n");
          NIASHDBG (DBG_MSG, "sane_read: read after end of buffer\n");
          s->fCancelled = SANE_FALSE;
          s->fScanning = SANE_FALSE;
          return SANE_STATUS_EOF;
        }
  ...
  if (s->fScanning)
    {
      ...
      NIASHDBG (DBG_MSG, "sane_cancel: freeing buffers\n");
    }
...
{
  NIASHDBG (DBG_MSG, "sane_set_io_mode %s\n", m ? "non-blocking" : "blocking");
...
{
  NIASHDBG (DBG_MSG, "sane_select_fd\n");
