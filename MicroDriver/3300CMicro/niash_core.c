/*
  ...
  $Id: niash_core.c,v 1.12 2005/01/16 09:39:40 ullsig Exp $
*/
  ...
  if (!pHWParams->fGamma16)
    {
      ...
      NIASHDBG (DBG_MSG, "  NIASH version < 00014\n");
    }
  else
    {
...
NiashOpen (THWParams * pHWParams, const char *pszName)
{
  ...
  switch (pHWParams->eModel)
    ...
    case eHp3300c:
      NIASHDBG (DBG_MSG, "Setting params for Hp3300\n");
    ...
    case eHp3400c:
    case eHp4300c:
      NIASHDBG (DBG_MSG, "Setting params for Hp3400c/Hp4300c\n");
    ...
    case eAgfaTouch:
      NIASHDBG (DBG_MSG, "Setting params for AgfaTouch\n");
    ...
    case eUnknownModel:
      NIASHDBG (DBG_MSG, "Setting params for UnknownModel\n");
      break;

    default:
      NIASHDBG (DBG_ERR, "ERROR: internal error! (%d)\n", (int) pHWParams->eModel);
      return -1;
    }
  ...
  if (!_ProbeRegisters (pHWParams))
    {
      NIASHDBG (DBG_ERR, "_ProbeRegisters failed!\n");
      return -1;
    }
  return 0;
}
...
CalcGamma (unsigned char *pabTable, double Gamma)
{
  ...
  for (i = 0; i < 4096; i++)
    {
      iData = (int) floor (256.0 * pow (((double) i / 4096.0), 1.0 / Gamma));
      pabTable[i] = (unsigned char) iData;
    }
}
...
InitScan (TScanParams * pParams, THWParams * pHWParams)
{
  ...
  switch (pParams->iDpi)
    ...
    case 600:
      break;
    default:
      NIASHDBG (DBG_ERR, "Invalid dpi (%d)\n", pParams->iDpi);
      return SANE_FALSE;
    }
  ...
  if (iHeight <= 0)
    {
      NIASHDBG (DBG_ERR, "Invalid height (%d)\n", iHeight);
      return SANE_FALSE;
    }

  if (pParams->iWidth <= 0)
    {
      NIASHDBG (DBG_ERR, "Invalid width (%d)\n", pParams->iWidth);
      return SANE_FALSE;
    }
  ...
  switch (pParams->iLpi)
    ...
    case 600:
      break;
    default:
      NIASHDBG (DBG_ERR, "Invalid lpi (%d)\n", pParams->iLpi);
      return SANE_FALSE;
    }
  ...
  if ((iExpTime % 8) != 0)
    {
      NIASHDBG (DBG_ERR, "Invalid exposure time (%d)\n", iExpTime);
      return SANE_FALSE;
    }
		   ...
		   SANE_Bool fReturn)
{
  ...
  if (p->iCurLine == 0)
      ...
      if (p->iLinesLeft > 0 && p->iLinesLeft <= iLines)
	{
	  iLines = p->iLinesLeft;
	  NIASHDBG (DBG_MSG, "\n");
	  NIASHDBG (DBG_MSG, "last bulk read\n");
...
XferBufferExit (TDataPipe * p)
{
  if (p->pabXferBuf != NULL)
    ...
    {
      NIASHDBG (DBG_ERR, "XferBufExit: Xfer buffer not initialised!\n");
    }
}
		...
		int iScaleDownDpi, int iScaleDownLpi)
{
  ...
  NIASHDBG (DBG_MSG, "_iScaleDown (Dpi,Lpi) = (%d,%d)\n", p->iScaleDownDpi,
       p->iScaleDownLpi);
  NIASHDBG (DBG_MSG, "_iBytesPerLine = %d\n", p->iBytesPerLine);
  NIASHDBG (DBG_MSG, "_iLinesPerCircBuf = %d\n", p->iLinesPerCircBuf);
  p->pabCircBuf =
  ...
  NIASHDBG (DBG_MSG, "Allocated %d unsigned chars for circular buffer\n",
  ...
  if (iHeight < 0)
    {
      ...
      NIASHDBG (DBG_MSG, "using unchecked XFER_BUF_SIZE\n");
      NIASHDBG (DBG_MSG, "_iXFerSize = %d\n",
...
#define MAX_LINES_PER_XFERBUF 800
	...
	_OptimizeXferSize (p->iLinesLeft, p->iLinesPerXferBuf);
      ...
      NIASHDBG (DBG_MSG, "_iXFerSize = %d for %d transfer(s)\n",
...
CircBufferExit (TDataPipe * p)
{
  ...
  if (p->pabCircBuf != NULL)
    ...
    {
      NIASHDBG (DBG_ERR, "CircBufferExit: Circular buffer not initialised!\n");
    }
}
		...
		unsigned char *pabCalWhite)
{
  ...
  NIASHDBG (DBG_MSG, "Black level (%d,%d,%d), White level (%d,%d,%d)\n",
  ...
  if (pabCalWhite)
    {
      pabCalWhite[0] = (unsigned char)  iWhiteR;
      pabCalWhite[1] = (unsigned char)  iWhiteG;
      pabCalWhite[2] = (unsigned char)  iWhiteB;
    }
  return SANE_TRUE;
}
